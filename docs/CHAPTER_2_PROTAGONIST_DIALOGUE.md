...
> 狀態：待審核，尚未接入遊戲。範圍從 Chapter 1 讀完 ARC_184 的回覆開始，到主角確認封存檔只能由 Lumen Arc 開啟、進入 Chapter 3 為止。
...
- `Skyline256_LAOS_Final.ipa` 的完整檔名。
- `.ipa` 一定是正確分類。
...
| 玩家事件 | 顯示台詞 |
|---|---|
| Chapter 1 結束、進入 Chapter 2 | `Lumen Arc. Recalled hardware, a dead operating system, and an impossible score.` / `If the current game cannot do it, I need the version ARC_184 was running.` |
...
| 聚焦封存站的檔名搜尋 | `I do not know the exact name yet.` / `Platform first. Filename second.` |
...
玩家可以在封存站搜尋檔名，但獨白不能因玩家提前輸入未取得的答案而替他確認未來知識。

| 提前或無關輸入 | 回應方向 | 台詞草稿 |
|---|---|---|
| 空白搜尋 | 保持輕微吐槽 | `Searching every filename at once. Technically efficient.` |
| 直接輸入完整 `Skyline256_LAOS_Final.ipa` | 不解釋來源、不額外提示格式 | `That is extremely specific.` / `I would like to know where I supposedly learned it.` |
...
1. 主角是否應在 Chapter 2 結尾明確說出 `Mom`，或採用較含糊的 B。
...
草稿核准後才建立 Chapter 2 專用 dialogue helper，並只接入以下事件：章節進入、首頁、Browser、Trending lead、Archive landing、搜尋框、五個格式標籤、檔案紀錄、相容性錯誤、錯誤 App 與重複操作。
