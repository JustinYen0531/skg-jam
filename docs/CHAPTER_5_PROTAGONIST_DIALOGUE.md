...
> 狀態：已接入 runtime，待人工審核台詞語氣。範圍從 Chapter 4 確認遊戲真名 `SKG` 開始，到主角搜尋 SKG、穿過 SKG Automation 那層無人的 2026 企業外殼、把 Snapshot 時間軸拖回 2014、看見被覆蓋掉的原始工作室 Silver Kite Games 與開發者 Noah Kade、進入 Chapter 6 為止。
...
| 拖到 2014、載入 Silver Kite Games 站台（尚未通關） | `There. Before the automation buried it.` / `Silver Kite Games. A person actually made this.` |
| 點第一處 `Noah Kade`：Studio design & code | `Noah Kade. Studio design and code.` / `One name, finally.` |
| 點第二處 `Noah Kade`：Completion build maintainer | `Noah Kade again. He maintained the completion build.` / `Two references. Not a stray byline.` |
| 點第三處 `Noah Kade`：Co-founder & lead designer，完成 Chapter 5 | `Co-founder and lead designer. Every surviving credit points to Noah Kade.` / `Noah Kade. That is where I go next.` |

拖到 2014 只揭露 Silver Kite Games，不完成 Chapter 5。玩家必須在同一頁找到並點擊三處 `Noah Kade` 署名；前兩處建立「同一人反覆出現」的確認感，第三處揭露他是共同創辦人與首席設計師，才完成章節並把名字交給 Chapter 6。重複點同一處只回應一次，不能重複計數。
...
Snapshot 時間軸橫跨 2009–2026，但只有 2014 真的存到頁面，其餘每一年都是空的（`no snapshot`）。玩家拖到錯的年份時，用以下短句把他推向「只有某一年存了東西」的直覺，但不直接說出 2014。

- 拖到 2015–2025（較新）：`Newer snapshots. Same automated nothing, just fresher.`
- 拖到 2009–2013（更早，仍空）：`Nothing saved before it existed. Keep sliding until a page actually loads.`
- 拖到最底 2009：`The reel bottoms out on an empty year. Too early to have left a trace.`
- 拖過中間任一空年：`The label ticks. The page doesn't. Only one year here actually kept anything.`

## 2014 站台上的閱讀

載入 2014 的 Silver Kite Games 站台後（此時仍在 Chapter 5），玩家會往下讀部落格、留言板與封存下載區。主角在這裡不擴寫、不煽情，語氣安靜；三處可點擊的 `Noah Kade` 署名是本章最後的確認鏈，點齊後才交給 Chapter 6。

| 玩家讀到 | 台詞草稿 |
|---|---|
| 站台標題 `SILVER KITE GAMES` | `Silver Kite Games. The name the corporate version scrubbed off.` |
| 第一處 `Studio design & code: Noah Kade` | `Noah Kade. Studio design and code.` / `One name, finally.` |
| 第二處 `Completion build maintained by Noah Kade` | `Noah Kade again. He maintained the completion build.` / `Two references. Not a stray byline.` |
| 第三處 `Co-founder & lead designer: Noah Kade` | `Co-founder and lead designer. Every surviving credit points to Noah Kade.` / `Noah Kade. That is where I go next.` |
...
以下三版都停在「拖回 2014、看見 Silver Kite Games、點齊三處署名、確認真人身分，並記下一個能往下查的名字 Noah Kade」，不得解讀回收全貌、`m_k`、隱藏路線，或任何結局。
...
4. 章節完成點定義為「在 2014 Silver Kite Games 站台點齊三處不同的 `Noah Kade` 署名」；載入 2014 本身不得推進，重複點同一署名不得重複計數。
...
- Snapshot 時間軸（`#wayback-scrubber`／`#wayback-year-label`）：拖到 2014 以外年份的〈沒有快照〉輪替；拖到 2014 只播放揭露句並載入 `#indie-2014`，不得完成章節。
- 2014 站台（`#indie-2014`／`#indie-blogs`／`#indie-guestbook`／`#browser-archive-box`）的閱讀句、死連結碎片輪替，以及 `studio-credit`、`developer-note`、`cofounder-credit` 三處署名事件。
- 錯誤 App 與重複操作的陪伴輪替句。
...
- 只有在 2014 站台點齊三處不同的 `Noah Kade` 署名才完成 Chapter 5（與 `completePuzzleChapter(prev, 5, …)` 一致）；載入 2014、重複署名、disambiguation 誘餌、企業殼、bot、空年份都不得推進章節。
...
7. 操作 Snapshot 時間軸：先拖到 2020、2010、拉到最底 2009，確認顯示 `no snapshot` 與〈沒有快照〉輪替、頁面不變；再拖到 2014，確認只載入 Silver Kite Games 站台，仍停在 Chapter 5。
8. 在 2014 站台往下讀：先依序點前兩處 `Noah Kade`，確認仍在 Chapter 5；重複點同一處確認不重複計數；再點 footer 的第三處，確認顯示共同創辦人／首席設計師收束句並進入 Chapter 6。另點 `Why 256`、回收段落、Forum offline、缺圖與損毀留言，確認都有短回饋且不提前解讀 Elias、`m_k` 或隱藏路線。
