...
export const drawChapterTenWorld = (
  ...
  const pixel = getPixelPresence(score);
  ...
  ctx.save();
  ctx.globalAlpha = 0.24 + pixel * 0.5;
  ctx.fillStyle = '#79a9a1';
  ctx.fillRect(0, height * 0.72, width, height * 0.28);
  ctx.fillStyle = '#172d35';
  for (let x = -((frame * 0.5) % 72); x < width; x += 72) {
    const skylineHeight = 18 + ((Math.floor(x / 72) % 4 + 4) % 4) * 12;
    ctx.fillRect(x, height * 0.72 - skylineHeight, 56, skylineHeight);
  }
  ctx.restore();
  ...
  const shell = getShellOpacity(score);
  if (shell > 0) {
    ctx.save();
    ctx.globalAlpha = shell * 0.55;
    ctx.fillStyle = '#6d28d9';
    for (let i = 0; i < 9; i += 1) {
      const x = (i * 137 - frame * (1 + i * 0.04)) % (width + 100);
      ctx.fillRect(x, 20 + (i % 4) * 88, 34 + (i % 3) * 12, 5);
    }
    ctx.restore();
  }
};
...
export const drawChapterTenPipe = (
  ...
  ctx.save();
  ctx.lineWidth = form === 'data-boundary' ? 1 : 2;
  ctx.strokeStyle = form === 'data-boundary' ? 'rgba(205,220,216,.62)' : '#708f70';
  ctx.fillStyle = form === 'pixel' || form === 'pixel-with-box' ? '#426b48' : 'rgba(0,0,0,.08)';
  ...
  if (form === 'pixel-with-box') {
    ctx.setLineDash([3, 3]);
    ctx.strokeStyle = 'rgba(230,238,230,.65)';
...
export const drawChapterTenBird = (
  ...
  if (form === 'pixel-bird' || form === 'outlined-bird') {
    ...
    ctx.fillRect(x - 11, y - 8, 18, 16);
    ctx.fillStyle = '#e8d88b';
    ctx.fillRect(x - 5, y - 11, 12, 5);
    ctx.fillStyle = '#1b2528';
    ctx.fillRect(x + 2, y - 7, 3, 3);
    ctx.fillStyle = '#b86135';
    ctx.fillRect(x + 7, y - 2, 8, 4);
    if (form === 'outlined-bird') {
      ctx.strokeStyle = '#dce7e2';
      ctx.strokeRect(x - 13, y - 13, 30, 26);
    }
    ...
    ctx.beginPath();
    ctx.arc(x, y, form === 'coordinate' ? 2.5 : 4, 0, Math.PI * 2);
    ctx.fill();
    if (form === 'coordinate') {
      ctx.font = '9px "JetBrains Mono"';
      ctx.fillText(`(${Math.round(x)},${Math.round(y)})`, x + 8, y - 7);
    }
  }
  ctx.restore();
};
...
export const drawChapterTenRoutePoint = (
  ...
  ctx.beginPath();
  ctx.arc(pipeX + 25, point.y, collected ? 2 : 4, 0, Math.PI * 2);
  ctx.fill();
  ctx.font = '7px "JetBrains Mono"';
  ctx.fillText(String(point.altitude), pipeX + 32, point.y + 2);
  ctx.restore();
};
...
export const drawChapterTenHud = (
  ...
  ctx.save();
  ctx.fillStyle = 'rgba(4,10,13,.76)';
  ctx.fillRect(14, 14, 190, 42);
  ctx.fillStyle = '#dce7e2';
  ctx.font = '10px "JetBrains Mono"';
  if (getHudMode(score) === 'distance-to-end') {
    ctx.fillText(`DISTANCE TO END  ${distanceToEnd(score)}`, 24, 31);
  } else {
    ctx.fillText(`LOCAL RANK  ARC_184`, 24, 31);
  }
  ctx.fillStyle = '#8fa8a0';
  ctx.fillText(`TRACE ${collected}/${required}`, 24, 47);
  ctx.textAlign = 'right';
  ctx.fillText(String(score).padStart(3, '0'), width - 20, 31);
  ctx.restore();
};
...
export const drawChapterTenBeat = (
  ...
  if (phase === 'restored-2013' && score > 165) {
    ...
    CHAPTER_TEN_DEV_RESIDUE.forEach((line, i) => ctx.fillText(line, width - 170, 28 + i * 13));
    ctx.restore();
  }
  if ((phase === 'memory-184' || beatFrames > 0) && beatFrames < 210) {
    ctx.save();
    ctx.fillStyle = 'rgba(3,7,9,.84)';
    ctx.fillRect(width / 2 - 118, height / 2 - 58, 236, 116);
    ctx.textAlign = 'center';
    ctx.font = '11px "JetBrains Mono"';
    CHAPTER_TEN_MEMORY_LINES.forEach((line, i) => {
      ctx.fillStyle = i === 2 ? '#f0ddb0' : '#c8d5d1';
      ctx.fillText(line, width / 2, height / 2 - 30 + i * 19);
    });
    ctx.restore();
  }
  if (phase === 'terminal-256') {
    ...
    ctx.fillRect(width - 128, 0, 128, height);
    ctx.translate(width - 64, height / 2);
    ctx.rotate(-Math.PI / 2);
    ctx.textAlign = 'center';
    ctx.fillStyle = '#b74d46';
    ctx.font = '12px "JetBrains Mono"';
    ctx.fillText(CHAPTER_TEN_TERMINAL_LABEL, 0, 4);
    ctx.restore();
  }
};
...
export const drawChapterTenComplete = (
  ...
  const overflow = CHAPTER_TEN_SCORE_OVERFLOW[Math.min(
    CHAPTER_TEN_SCORE_OVERFLOW.length - 1,
    Math.floor(frame / 45),
  )];
